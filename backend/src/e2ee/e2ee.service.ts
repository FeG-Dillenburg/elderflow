...
export class E2eeService {
  ...
  async keyState(user: User) {
    ...
    return {
      ...
      sharedPassphraseSlot: state.sharedPassphraseSlot.toString('base64url'),
      contentKeyWrapper: state.contentKeyWrapper.toString('base64url'),
    };
  }
  ...
  async recoveryMetadata(user: User) {
    ...
    return {
      ...
      recoverySlot: state.recoverySlot.toString('base64url'),
    };
  }
  ...
  async recoveryCeremony(user: User, id: string) {
    ...
    return {
      ...
      candidateSharedPassphraseSlot: ceremony.candidateSharedPassphraseSlot.toString('base64url'),
  ...
  async startRecovery(user: User, input: StartRecoveryDto) {
    this.assertKeyOperator(user);
  ...
  async approveRecovery(user: User, id: string, input: ApproveRecoveryDto) {
    this.assertKeyOperator(user);
    return this.dataSource.transaction(async (manager) => {
  ...
  async activateRecovery(user: User, id: string): Promise<{ activated: true; generation: number }> {
    this.assertKeyOperator(user);
    return this.dataSource.transaction(async (manager) => {
      const ceremony = await this.lockCeremony(manager, id);
      ...
      if (![ceremony.initiatorId, ceremony.approverId].includes(user.id)) {
  ...
  async abortRecovery(user: User, id: string): Promise<void> {
    this.assertKeyOperator(user);
  ...
  private assertContentUser(user: User): void {
    if (!['superadmin', 'admin', 'user'].includes(user.role)) {
  ...
  private decodeKeyEnvelope(value: string, state: E2eeKeyState): Buffer {
    const decoded = this.decodeEnvelope(value);
