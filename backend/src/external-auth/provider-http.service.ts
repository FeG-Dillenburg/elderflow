...
export class ProviderHttpService {
  ...
  async getJson(url: string, authorization?: string, stage?: ProviderRequestStage): Promise<Record<string, unknown>> {
    return this.withStage(stage, () => this.requestJson(url, { headers: authorization ? { Authorization: authorization } : undefined }));
  }
  ...
  async postForm(url: string, form: URLSearchParams, authorization?: string, stage?: ProviderRequestStage): Promise<Record<string, unknown>> {
    return this.withStage(stage, () => this.requestJson(url, {
      method: 'POST',
  ...
  private request(url: URL, init: RequestInit): Promise<{ status: number; location?: string; body: string }> {
    return new Promise((resolve, reject) => {
      ...
      request.on('error', (error) => reject(error instanceof HttpException
        ? error
        : codedHttpException(HttpStatus.BAD_GATEWAY, providerNetworkFailureCode(error), 'External login provider request failed')));
      if (body) request.write(body);
      request.end();
    });
  }

  private async withStage(stage: ProviderRequestStage | undefined, request: () => Promise<Record<string, unknown>>): Promise<Record<string, unknown>> {
    try {
