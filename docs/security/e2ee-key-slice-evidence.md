...
Open `/setup`, enter the one-time backend setup password, create the first Superadmin and a distinct shared passphrase, print the displayed `EFR1.` Recovery Secret twice, verify both complete paper copies, and complete setup. The submitted setup request contains the three deterministic-CBOR wrappers, public UUID/version metadata, and `custodyCopiesAcknowledged: 2`; it contains neither the Recovery Secret nor an unwrapped key.
...
The machine-readable vectors are in [`e2ee-v1-key-vectors.json`](./e2ee-v1-key-vectors.json). They include unchanged RFC 5869 and RFC 8032 vectors plus the approved complete recovery wrapper bytes.
