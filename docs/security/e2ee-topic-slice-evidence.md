...
- The running-instance create phase passed. The backend process was stopped and started against the same database; the verify phase then decrypted all four exact Topic names and the standalone Update successfully. The runner now sends encrypted requests without the legacy development-unlock header so future executions exercise the production-style write gate.
