...
Before accepting a proposal, the production relay coordinates a short-lived document barrier with content-free control frames. `request-compaction(triggerServerSequence)` opens the barrier and broadcasts `compaction-barrier(barrierId)`, including when later encrypted envelopes are already queued. Each client stops creating envelopes, drains envelopes already created, and sends `compaction-drained(barrierId)`. The designated client freezes a compaction document at barrier entry, applies drained remote updates to that copy, and excludes its own post-barrier plaintext deltas. When every connected participant is drained, the relay reads the stable PostgreSQL sequence and sends `compaction-ready(barrierId, serverSequence)` only to the designated requester. The binary snapshot POST includes `X-ElderFlow-Compaction-Barrier-Id`; the backend requires the same authenticated user, a ready barrier, and exact agreement between the barrier sequence, signed snapshot sequence, and locked document sequence.

Successful commit and all abort paths broadcast `compaction-released(barrierId, outcome)`. Timeout, disconnect before commit, `compaction-failed`, validation failure, or stale compare-and-swap aborts against the unchanged parent. Once commit is claimed, bounded PostgreSQL lock and statement timeouts keep the database phase finite, the claim is verified before the transaction may commit, and no independent barrier timer can race a successful commit. A joining client is included while draining or held paused once commit starts. Non-WebSocket document writes abort before commit or receive `E2EE_COMPACTION_IN_PROGRESS` during commit. Post-barrier Yjs deltas remain plaintext only in volatile unlocked memory and are encrypted after release and resynchronization. Old-parent rejection recovery first decrypts the locally held envelope, resynchronizes, registers a fresh client epoch/nonce prefix, and only then reseals. Pending envelopes carry their original epoch and signing-key identity so concurrent providers can still decrypt and reseal them after a shared-session rotation, and all epochs created by the unlock session are revoked at lock even if registration completes concurrently.
