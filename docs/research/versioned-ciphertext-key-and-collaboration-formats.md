...
Before accepting a proposal, the production relay coordinates a short-lived document barrier with content-free control frames. `request-compaction(triggerServerSequence)` opens the barrier and broadcasts `compaction-barrier(barrierId)`. Each client stops creating envelopes, drains envelopes already created, and sends `compaction-drained(barrierId)`. When every connected participant is drained, the relay reads the stable PostgreSQL sequence and sends `compaction-ready(barrierId, serverSequence)` only to the designated requester. The binary snapshot POST includes `X-ElderFlow-Compaction-Barrier-Id`; the backend requires the same authenticated user, a ready barrier, and exact agreement between the barrier sequence, signed snapshot sequence, and locked document sequence.

Successful commit and all abort paths broadcast `compaction-released(barrierId, outcome)`. Timeout, disconnect before commit, `compaction-failed`, validation failure, or stale compare-and-swap aborts against the unchanged parent. A joining client is included while draining or held paused once commit starts. Post-barrier Yjs deltas remain plaintext only in volatile unlocked memory and are encrypted after release and resynchronization. Old-parent rejection recovery first decrypts the locally held envelope, resynchronizes, registers a fresh client epoch/nonce prefix, and only then reseals; all epochs created by the unlock session are revoked at lock.
